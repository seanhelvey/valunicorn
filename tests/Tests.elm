...
all =
    describe "The Calculator Module"
        [ describe "aggregateFutureValues"
            [ test "Zero periods" <|
                \() ->
                    Expect.equal (Nonempty.get -1 <| aggregateFutureValues 0.0 0.0 2.65 0.062 0) 2.65
            , test "One period" <|
                \() ->
                    Expect.equal (Nonempty.get -1 <| aggregateFutureValues 0.0 0.0 2.65 0.062 1) 2.8143000000000002
            , test "Five periods" <|
                \() ->
                    Expect.equal (Nonempty.get -1 <| aggregateFutureValues 0.0 0.0 2.65 0.062 5) 15.927942909494408
            , test "Ten periods" <|
                \() ->
                    Expect.equal (Nonempty.get -1 <| aggregateFutureValues 0.0 0.0 2.65 0.062 10.0) 37.44497036944294
            ]
